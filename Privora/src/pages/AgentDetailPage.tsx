...
import { getAgent, getExecutions, getPrivacy, deleteAgent, runAgent } from "@/lib/api";
...
export default function AgentDetailPage() {
  ...
  const loadAgent = async () => {
    ...
    try {
      ...
      const exec = await getExecutions(id);
setExecutions(exec);

if (exec.length > 0) {
  setResults(exec[exec.length - 1]);
}
const privacyData = await getPrivacy(id);
setPrivacy(privacyData);
  ...
  const filteredLogs = useMemo(() => {
    let logsData: LogEntry[] = logs;
    if (logFilter !== "All") logsData = logsData.filter((l) => l.level === logFilter);
    if (logSearch) {
      const q = logSearch.toLowerCase();
      logsData = logsData.filter((l) => l.message.toLowerCase().includes(q));
    }
    return logsData;
  }, [logFilter, logSearch]);
...
await runAgent(agent.id, selectedFile)
    setLogs((prev) => [
  {
    ...
    const exec = await getExecutions(agent.id)
setExecutions(exec)
...
const privacyData = await getPrivacy(agent.id)
setPrivacy(privacyData)
...
Completed at {results?.completed 
 ? new Date(results.completed).toLocaleString() 
 : "—"}
</p>
              </div>
              <div className="bg-secondary rounded-md p-4">
                <p className="text-xs text-muted-foreground">Duration</p>
                <p className="text-sm font-semibold text-foreground mt-1">{results?.duration || "0s"}</p>
              </div>
              <div className="bg-secondary rounded-md p-4">
                <p className="text-xs text-muted-foreground">Rows Processed</p>
                <p className="text-sm font-semibold text-foreground mt-1">{results?.rowsProcessed || 0}</p>
              </div>
            </div>
  ...
  <tr className="border-b border-border">
    <th className="text-left py-2 text-xs text-muted-foreground">Metric</th>
    <th className="text-left py-2 text-xs text-muted-foreground">Value</th>
    <th className="text-left py-2 text-xs text-muted-foreground">Info</th>
    <th className="text-left py-2 text-xs text-muted-foreground">Remarks</th>
  </tr>
</thead>
                <tbody>
{results ? (
  <>
    <tr className="border-b border-border/50">
      <td className="py-2 text-foreground">Rows</td>
      <td className="py-2">{results.result?.rows}</td>
      <td>-</td>
      <td>-</td>
    </tr>

    <tr className="border-b border-border/50">
      <td className="py-2 text-foreground">Columns</td>
      <td className="py-2">{results.result?.columns}</td>
      <td>-</td>
      <td>-</td>
    </tr>

    <tr className="border-b border-border/50">
  <td className="py-2 text-foreground">Dataset Type</td>
  <td colSpan={3} className="py-2 whitespace-pre-wrap">
    {results.result?.insights?.dataset_type || "-"}
  </td>
</tr>

<tr className="border-b border-border/50">
  <td className="py-2 text-foreground">Key Insights</td>
  <td colSpan={3} className="py-2 whitespace-pre-wrap">
    {results.result?.insights?.key_insights || "-"}
  </td>
</tr>
...
<tr className="border-b border-border/50">
  ...
  <td colSpan={3} className="py-2 whitespace-pre-wrap">
    {results.result?.insights?.data_quality || "-"}
  </td>
</tr>
  </>
) : (
    ...
    <td colSpan={4} className="py-4 text-center text-muted-foreground">
      No Results Yet
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => { navigator.clipboard.writeText(JSON.stringify(results)); toast.success("Copied!"); }} className="flex items-center gap-1.5 px-3 py-2 rounded-md bg-secondary text-foreground text-xs hover:bg-accent transition">
...
{new Date(ex.started).toLocaleString("en-IN", {
  timeZone: "Asia/Kolkata"
})}
</td>

<td className="py-2 text-foreground">
{new Date(ex.completed).toLocaleString("en-IN", {
  timeZone: "Asia/Kolkata"
})}
</td>
        ...
        {tab === 3 && (
          <div className="space-y-8">
            ...
            <div className="bg-secondary rounded-lg p-5 space-y-3">
              ...
              {[
                { label: "Proof ID", value: privacy?.agent_id || "-", mono: true },
 { label: "Proof Hash", value: privacy?.proof || "-", mono: true },
 { label: "Proof Type", value: "SHA-256 Hash" },
 { label: "Generated", value: new Date(privacy?.timestamp).toLocaleString() || "-" },
 { label: "Verified", value: new Date(privacy?.timestamp).toLocaleString() || "-" },
              ].map((item) => (
