...
import { addAgent, generateId, type Agent } from "@/lib/mock-data";
...
export default function CreateAgentPage() {
  ...
  const [type, setType] = useState<Agent["type"]>("Data Processing");
  ...
  const handleCreate = async () => {
    ...
    const agent: Agent = {
      ...
      type,
      status: "paused",
      privacyStatus: proofGen ? "pending" : "unverified",
      privacyMode,
      lastRun: "Never",
      created: new Date().toISOString().split("T")[0],
      executions: 0,
      dataSource: { type: dataSourceTab, details: getDataSourceDetails() },
      encryption: encrypt,
      proofGeneration: proofGen,
    };
    addAgent(agent);
    setLoading(false);
    toast.success(`Agent created! ID: ${agent.id}`);
              ...
              {dataSourceTab === "file" && (
                <div className="space-y-3">
                  <div
                    ...
                    onClick={() => {
                      setFileName("sample_data." + fileType.toLowerCase().replace(/[^a-z]/g, ""));
                      toast.info("File simulated: sample_data." + fileType.toLowerCase().replace(/[^a-z]/g, ""));
                    }}
                  >
                    ...
                    <button className="mt-3 px-4 py-1.5 rounded text-xs bg-secondary text-foreground hover:bg-accent transition">
                      Browse
                    </button>
                  </div>
                  <select
